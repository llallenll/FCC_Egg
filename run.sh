#!/bin/sh
# /run.sh -- the egg's startup script (the panel starts the VPS with ./run.sh).
# This is ysdragon/Pterodactyl-VPS-Egg scripts/run.sh, kept in the FCC_Egg repo so
# autorun.sh can update it together with itself (see the update check in autorun.sh).
# Changes to this file take effect on the next start of the server.
